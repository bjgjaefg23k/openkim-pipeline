...
module iron_pair_potential__MO_978045385159_000
...
double precision, parameter :: model_cutoff  =  3.44
...
!                                                ! value equal to `value' (a number)
!-------------------------------------------------------------------------------
double precision, parameter :: a0 = -0.2002108
double precision, parameter :: a1 = -0.5047747
double precision, parameter :: a2 = 1.372738
double precision, parameter :: a3 = -15.09618
double precision, parameter :: a4 = -12.90021
double precision, parameter :: b0 = -1.581570
double precision, parameter :: b1 =  0.477871
double precision, parameter :: b2 = -0.639230
double precision, parameter :: c0 =  -0.1469636
double precision, parameter :: c1 = 0.4521426
double precision, parameter :: c2 = 0.2221241
double precision, parameter :: c3 = 1.725326
double precision, parameter :: c4 = -12.91063
double precision, parameter :: c5 = 14.67111
double precision, parameter :: aa0 = 8752.934
double precision, parameter :: B =  4.572488
contains
...
if (r <  1.90) then
   ...
   phi = a0*(r-2.4)**0 +  a1*(r-2.4)**1 + a2*(r-2.4)**2 + a3*(r-2.4)**3 + a4*(r-2.4)**4

else if (  r < 3.00) then

   phi = b2*(r-3.115829)**3 +  b1*r +  b0

else if ( r <  3.44) then

   phi = c0*(r-3.0)**0 +  c1*(r-3.0)**1 + c2*(r-3.0)**2 + c3*(r-3.0)**3 + c4*(r-3.0)**4 + c5*(r-3.0)**5
...
if (r <  1.90) then
   ...
   Phi = a0*(r-2.4)**0 +  a1*(r-2.4)**1 + a2*(r-2.4)**2 + a3*(r-2.4)**3 + a4*(r-2.4)**4
   dphi = a1 + 2*a2*r + 3*a3*(r-2.4)**2 + 4*a4*(r-2.4)**3

else if (  r < 3.00) then

   phi = b2*(r-3.115829)**3 +  b1*r +  b0
   dphi = 3*b2*(r-3.115829)**2 + b1

else if ( r <  3.44) then

   phi = c0*(r-3.0)**0 + c1*(r-3.0)**1 + c2*(r-3.0)**2 + c3*(r-3.0)**3 + c4*(r-3.0)**4 + c5*(r-3.0)**5
dphi = c1 + 2* c2*(r-3.0) + 3*c3*(r-3.0)**2  + 4*c4*(r-3.0)**3 + 5*c5*(r-3.0)**4
...
end module iron_pair_potential__MO_978045385159_000
...
!
!-------------------------------------------------------------------------------
integer function iron_pair_potential__MO_978045385159_000_init(pkim)
use iron_pair_potential__MO_978045385159_000
use KIM_API
implicit none
...
ier = KIM_STATUS_OK
42 continue
iron_pair_potential__MO_978045385159_000_init = ier
return

end function iron_pair_potential__MO_978045385159_000_init
